...
def find_neighbours2(curr_node, maze, all_moves):
    ...
    list_to_traverse = list_of_moves.copy()
    for x in list_to_traverse:
        if (maze[x[0]][x[1]] == 3 or maze[x[0]][x[1]] == 1 or maze[x[0]][x[1]] == -1 or x in all_moves):
            list_of_moves.remove(x)
    return  list_of_moves
...
def ghost_movement(positions):
    ...
    for pos in positions:
        ...
        i = i + 1
    return positions, all_possible_pos
    ...
    for y in range(50):
        ...
        while (status == True):     
            ...
            if (m[agent_pos[0]][agent_pos[1]] == -1):
                    ...
                    plt.close('all')
                    break
            ghost_pos, all_poss_moves =  ghost_movement(ghost_pos)
            m = mog.copy()
        ...
        i = i + 1
    n = n + 10
data.to_csv('Agent4_2.csv', index=False)